...
export const SERVER_INSTRUCTIONS = [
  ...
  "",
  "Typical flow: task_list shows the queue, task_claim takes a card (send your executor: cli, exact session model from --model, session_id) and returns the contract once with a one-line measure.command (the project dossier is project_get's job; return: full gives the legacy markdown briefing); task_create with claim creates and takes a card in one call, then create and push a commit before task_deliver. A modified working tree is not a delivery; send the commit hash and branch with summary, evidence and usage. When the CLI recipe yields tokens_per_model, measure from the transcript; estimated: true is refused unless you send the reason the recipe prints when the transcript is missing. CLIs whose recipe yields no_tokens may estimate.",
