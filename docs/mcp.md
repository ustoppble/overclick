...
| `task_update` | progress, comment (`comment_kind` supports `report` to increment `reports_count`), `resolved_in` metadata (string or `null`), the `reviewed` mark, a new `harness`, or a `usage` block even after deliver/discard; managed tokens may send `status: descartado` with `superseded_by` |
